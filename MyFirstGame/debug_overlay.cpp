...
namespace mfg
{
	namespace core
	{
		DebugOverlay::DebugOverlay(Window& window, EntityManager& entmgr) : window_(window), entmgr_(entmgr)
		{
		...
		void DebugOverlay::draw(sf::RenderTarget& target, sf::RenderStates states) const
		{
			...
			if (mouse_position.x >= viewport.left && mouse_position.x <= (viewport.left + view.getSize().x))
			{
				...
				if (mouse_position.y >= viewport.top && mouse_position.y <= (viewport.top + view.getSize().y))
				{
					...
					sf::Text position_x_text(ss_x.str(), font_, 18);
					...
					sf::Text position_y_text(ss_y.str(), font_, 18);
					...
					/* entity information */
					auto group = entmgr_.getEntities().group<PositionComponent, SpriteComponent, ActorComponent>();

					/* interate over a group that returns the position, sprite and actor components */
					group.each([this, mouse_position, position_shape_x, position_shape_y](
						auto entity, PositionComponent& position, SpriteComponent& sprite, ActorComponent& actor)
						{
							if (mouse_position.x > position.getPosition().x && mouse_position.x <= (position
							                                                                        .getPosition().x +
								sprite.getLocalBounds().width))
							{
								if (mouse_position.y >= (position.getPosition().y - sprite.getLocalBounds().height) &&
									mouse_position.y <= position.getPosition().y)
								{
									/* entity shape */
									std::stringstream ss_entity_name;
									ss_entity_name << "" << actor.getName();
									sf::Text entity_name_text(ss_entity_name.str(), font_, 18);
									...
									const auto entity_shape_x = position_shape_x;
									const auto entity_shape_y = (position_shape_y - 10) - entity_shape
									                                                      .getLocalBounds().height;

									entity_shape.setPosition(entity_shape_x, entity_shape_y);
									entity_name_text.setPosition(entity_shape.getPosition().x + 10,
									                             entity_shape.getPosition().y + 10);

									this->window_.draw(entity_shape);
									this->window_.draw(entity_name_text);
								}
							}
						});

					window_.draw(position_shape);
					window_.draw(position_x_text);
					window_.draw(position_y_text);
				}
			}
		}
	}
